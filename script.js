/* ================================================================
   MindMate AI — Chatbot Engine  v2.0
   NLP-style keyword scoring, multi-turn topic focus,
   natural follow-up chains, sleep + motivation + career support
   ================================================================ */
...
function generateReply(text) {
  ...
  if (intent !== "unknown" && intent !== "greeting" && intent !== "gratitude") {
    if (!state.topic || switching) {
      ...
      /* mid-conversation different topic signal — gently acknowledge */
  ...
  switch (state.topic) {
    ...
    case "sadness":
      ...
      if (lower.match(/hurt myself|end it|disappear|don't want to be here|suicid|self.harm|self harm/))
        return r.crisis;
...
window.onload = () => {
  ...
  if (savedTheme==="dark") { document.body.classList.add("dark-mode"); if(themeToggle) themeToggle.textContent="☀️ Light Mode"; }
  else { if(themeToggle) themeToggle.textContent="🌙 Dark Mode"; }
};

if (themeToggle) {
  themeToggle.addEventListener("click", () => {
    document.body.classList.toggle("dark-mode");
    if (document.body.classList.contains("dark-mode")) { themeToggle.textContent="☀️ Light Mode"; localStorage.setItem("theme","dark"); }
...
function createNewChat() {
  ...
  chats.unshift(chat); currentChatId = chat.id;
  state = { topic:null, turn:0, lastBot:null };
  saveChats(); renderChatList(); loadChat(chat.id);
}
...
function loadChat(chatId) {
  ...
  state = { topic:null, turn:0, lastBot:null };
  chat.messages.forEach(msg => createMessage(msg.text, msg.sender));
  renderChatList();
}
...
function renderChatList() {
  ...
  chats.forEach(chat => {
    const li = document.createElement("li"); li.classList.add("chat-item");
    if (chat.id===currentChatId) li.classList.add("active-chat");
    const title = document.createElement("span"); title.className="chat-title"; title.textContent=chat.name; title.onclick=()=>loadChat(chat.id);
    const actions = document.createElement("div"); actions.className="chat-actions";
    const rb = document.createElement("button"); rb.textContent="✏"; rb.onclick=e=>{e.stopPropagation();renameChat(chat.id);};
    const db = document.createElement("button"); db.textContent="🗑"; db.onclick=e=>{e.stopPropagation();deleteChat(chat.id);};
    actions.appendChild(rb); actions.appendChild(db);
    li.appendChild(title); li.appendChild(actions); historyList.appendChild(li);
  });
}

function renameChat(chatId) {
  const chat = chats.find(c=>c.id===chatId);
  const n = prompt("Rename chat", chat.name); if(!n) return;
  chat.name=n; saveChats(); renderChatList();
}
...
function deleteChat(chatId) {
  ...
  if (!currentChatId) { if(chatContainer) chatContainer.innerHTML=""; if(welcomeScreen) welcomeScreen.style.display="block"; }
...
function sendMessage() {
  ...
  setTimeout(() => {
    hideTyping();
    const reply = generateReply(text); state.lastBot=reply;
    addMessage(reply,"bot");
    const chat = chats.find(c=>c.id===currentChatId);
    if (chat && chat.name==="New Chat" && chat.messages.filter(m=>m.sender==="user").length===1) {
      chat.name = text.length>32 ? text.substring(0,32)+"…" : text;
      saveChats(); renderChatList();
    }
  }, 900 + Math.random()*500);
}
...
function addMessage(text, sender) {
  ...
  chat.messages.push({text,sender}); saveChats();
  localStorage.setItem("mindmate_chat_count", (parseInt(localStorage.getItem("mindmate_chat_count")||"0")+1));
...
function showTyping() { if(typingIndicator) typingIndicator.classList.remove("hidden"); }
function hideTyping() { if(typingIndicator) typingIndicator.classList.add("hidden"); }

if (sendBtn) sendBtn.addEventListener("click", sendMessage);
if (messageInput) messageInput.addEventListener("keypress", e=>{ if(e.key==="Enter") sendMessage(); });
suggestionButtons.forEach(btn => { btn.addEventListener("click", ()=>{ if(messageInput){messageInput.value=btn.textContent;sendMessage();} }); });
